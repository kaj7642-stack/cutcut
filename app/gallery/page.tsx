    ...
    <main className="min-h-screen px-4 py-8 max-w-6xl mx-auto">
      ...
      <div className="text-center mb-12">
        <h1 className="text-3xl sm:text-4xl font-bold mb-3">
          AI가 만든 <span style={{ color: "var(--accent)" }}>게임 하이라이트</span> 갤러리
        </h1>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {filtered.map((item) => (
          <div key={item.id} className="card group" style={{ padding: 0, overflow: "hidden" }}>
            {/* Placeholder thumbnail */}
            <div
              className="aspect-video flex items-center justify-center text-4xl"
              style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)" }}
            >
              {item.game.includes("리그") ? "🎯" :
               item.game.includes("배틀") ? "🔫" :
               item.game.includes("발로") ? "🎯" :
               item.game.includes("오버") ? "⚔️" :
               item.game.includes("에이펙스") ? "🚀" : "🎮"}
            </div>
            <div className="p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full" style={{ border: "1px solid var(--border)", color: "var(--fg-muted)" }}>
                  {item.mode === "shorts" ? "📱 쇼츠" : "🖥️ 롱폼"}
                </span>
              </div>
      ...
      <div className="card text-center mb-8">
        ...
        <h2 className="text-xl font-bold mb-2">나만의 하이라이트를 만들어보세요</h2>
